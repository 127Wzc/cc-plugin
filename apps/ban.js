...
export class exampleBan extends plugin {
  ...
  async ban(e) {
    ...
    //按号码段过滤群机器人
    else if (e.at > 2854000000 && e.at < 2855000000) return false;
    else if (e.at > 3889000000 && e.at < 3890000000) return false;
    ...
    let newMsg = e.message
      .map((item) => {
        let returnMsg;
        //其他类型消息待定
        if (item.type == "text") returnMsg = item.text;
        else if (item.type == "image") returnMsg = item.name;
        else if (item.type == "mface") returnMsg = item.emoji_id;
        else returnMsg = item.name ? item.name : "";
    ...
    logger.debug(
      `当前群(${e.group_id})用户(${res.banID})已刷屏${res.msgNum}次`
    );
